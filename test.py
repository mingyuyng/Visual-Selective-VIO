...
from utils import data_partition, pose_accu
...
for test_video in opt.test_list:
    ...
    # Draw figures
    pose_eval.eval(test_video, dec_est, prob_est)

    np.save(os.path.join(opt.save_dir, opt.model_name, test_video + '_decisions.npy'), dec_est)
    np.save(os.path.join(opt.save_dir, opt.model_name, test_video + '_probs.npy'), prob_est)
